...
from hikkatl.tl.types import InputMediaWebPage, Message
...
class TestMod(loader.Module):
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                ...
                None,
                (
                    "Custom .ping message template. Available placeholders:"
                    " {ping}, {uptime}, {ping_hint}, {hostname}, {user}, {platform}."
            ...
            loader.ConfigValue(
                "ping_custom_hint",
                None,
                "Occasional extra line shown under the .ping result (overrides"
                " the default localized hint)",
                validator=loader.validators.Union(
                    loader.validators.String(),
                    loader.validators.NoneType(),
                ),
            ...
            loader.ConfigValue(
                "ping_banner_url",
                None,
                "Banner shown together with .ping",
                validator=loader.validators.Union(
            ...
            loader.ConfigValue(
                "ping_quote_media",
                False,
                "Show the .ping banner as a quoted webpage preview",
                validator=loader.validators.Boolean(),
            ),
            loader.ConfigValue(
                "ping_invert_media",
                False,
                "Show the .ping banner below the text instead of above it",
                validator=loader.validators.Boolean(),
            ),
        )
    ...
    def _ping_banner_kwargs(self) -> dict:
        if not self.config["ping_banner_url"]:
            return {}

        file = self.config["ping_banner_url"]
        if self.config["ping_quote_media"]:
            file = InputMediaWebPage(str(self.config["ping_banner_url"]), optional=True)

        return {"file": file, "invert_media": self.config["ping_invert_media"]}
    ...
    async def ping(self, message: Message):
        ...
        delta_ms = round((time.perf_counter_ns() - start) / 10**6, 3)
        hint = self.config["ping_custom_hint"] or self.strings("ping_hint")
        show_hint = random.choice([0, 0, 1]) == 1

        if self.config["ping_custom_message"]:
            data = {
                "ping": delta_ms,
                "uptime": utils.formatted_uptime(),
                "ping_hint": hint if show_hint else "",
            ...
            text = self.strings("results_ping").format(
                delta_ms,
                utils.formatted_uptime(),
            ) + (f"\n\n{hint}" if show_hint else "")

        await utils.answer(
            message,
            text,
            **self._ping_banner_kwargs(),
        )
