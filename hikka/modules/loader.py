...
from hikkatl.tl.types import Channel, InputMediaWebPage, Message
...
class LoaderMod(loader.Module):
        ...
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                ...
                lambda: (
                    ...
                    " its docstring) in the 'module loaded' message"
                ),
                validator=loader.validators.Boolean(),
            ),
        )
    ...
    def _loaded_banner_kwargs(self, doc: str, message: Message) -> dict:
        """kwargs to attach the module's own banner to the 'loaded' message"""
        if not self.config["show_banner"] or getattr(message, "document", None):
            return {}
        ...
        async with (dragon.import_lock if is_dragon else lambda _: FakeLock())(
            ...
            if any(
                ...
                await utils.answer(
                    ...
                    **(
                        {}
                        if subscribe_markup
                        else self._loaded_banner_kwargs(doc, message)
                    ),
                )
                return
            ...
            try:
                await utils.answer(
                    ...
                    loaded_msg(),
                    reply_markup=subscribe_markup,
                    **({} if subscribe_markup else self._loaded_banner_kwargs(doc, message)),
                )
