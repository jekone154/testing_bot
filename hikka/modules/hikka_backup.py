...
class HikkaBackupMod(loader.Module):
    ...
    async def backupall(self, message: Message):
        """Backup database and modules together, in a single archive"""
        db_bytes = json.dumps(self._db).encode()
        ...
        with zipfile.ZipFile(mods, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            zipf.writestr(
                "db_mods.json",
                json.dumps(self.lookup("Loader").get("loaded_modules", {})).encode(),
            )
        mods_bytes = mods.getvalue()

        archive = io.BytesIO()
        with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED) as z:
            z.writestr("db.json", db_bytes)
            z.writestr("mods.zip", mods_bytes)
    ...
    async def restoreall(self, message: Message):
        """Restore database and modules from a .backupall archive"""
        if not (reply := await message.get_reply_message()) or not reply.media:
            await utils.answer(message, self.strings("reply_to_file"))
            return
