...
from hikkatl.tl.types import Message, InputMediaWebPage
...
class Help(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "banner_url",
                None,
                lambda: "Banner shown above/below the general .help listing",
            ...
            loader.ConfigValue(
                "media_quote",
                False,
                lambda: "Show the .help banner as a quoted webpage preview",
                validator=loader.validators.Boolean(),
            ),
            loader.ConfigValue(
                "invert_media",
                False,
                lambda: "Show the .help banner below the text instead of above it",
                validator=loader.validators.Boolean(),
            ),
            loader.ConfigValue(
                ...
                True,
                lambda: (
                    "Show a module's own banner (declared via `# meta banner:` in"
                    " its docstring) in `.help <module>`"
                ),
                validator=loader.validators.Boolean(),
            ),
        )
    ...
    def _get_banner_url(self, doc: str):
        """Extract a per-module banner from a `# meta banner: <url>` doc tag"""
        match = re.search(r"# ?meta banner: ?(.+)", doc or "")
        return match.group(1).strip() if match else None

    def _help_banner_kwargs(self) -> dict:
        """kwargs for the general .help banner (config-driven)"""
        if not self.config["banner_url"]:
            return {}

        file = self.config["banner_url"]
        if self.config["media_quote"]:
            file = InputMediaWebPage(str(self.config["banner_url"]), optional=True)

        return {"file": file, "invert_media": self.config["invert_media"]}
    ...
    async def modhelp(self, message: Message, args: str):
        ...
        banner_url = (
            self._get_banner_url(module.__doc__)
            if self.config["show_preview_in_help"] and not is_dragon
            else None
        )
        ...
        if banner_url:
            await utils.answer_file(
                message,
                InputMediaWebPage(banner_url, optional=True),
                text,
                invert_media=True,
            )
        else:
    ...
    async def help(self, message: Message):
        ...
        if self.config["banner_url"]:
            banner_kwargs = self._help_banner_kwargs()
            await utils.answer_file(
                message,
                banner_kwargs.pop("file"),
                text,
                **banner_kwargs,
            )
        else:
