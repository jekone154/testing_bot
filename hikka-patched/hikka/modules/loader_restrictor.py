# ©️ Codrago, 2024-2030
# Originally part of Heroku Userbot: https://github.com/coddrago/Heroku
# Ported to Hikka Userbot. Distributed under the terms of the GNU AGPLv3
...
from hikkatl.tl.custom import Message
...
class LoaderRestrictor(loader.Module):
    """Requires a short quiz to be passed before third-party modules can be installed"""

    strings = {"name": "LoaderRestrictor"}

    async def client_ready(self):
        self.poll: typing.Optional[PollStatus] = None
    ...
    def _build_poll(self, poll_step: PollStep) -> InputMediaPoll:
        ...
        return InputMediaPoll(
            poll=poll,
            correct_answers=[str(poll_step.answer_index).encode()],
            solution=solution,
    ...
    async def poll_handler(self, update: UpdateMessagePollVote):
        ...
        try:
            chosen_index = int(update.options[0])
        except (IndexError, ValueError):
            return

        correct = chosen_index == QUESTIONS[self.poll.step].answer_index
    ...
    async def _start_quiz(self):
        ...
        if self.poll:
            await self.inline.bot.delete_messages(self._client.tg_id, self.poll.message_ids)
    ...
    async def _end_quiz(self):
        ...
        await self.inline.bot.delete_messages(self._client.tg_id, message_ids)
        await self.inline.bot.send_message(
            self._client.tg_id,
    ...
    async def bot_watcher(self, message: BotInlineMessage):
        if message.text != "/start lm_verify" or message.sender_id != self._client.tg_id:
            return
