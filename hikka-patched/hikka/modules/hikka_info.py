...
class HikkaInfoMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "quote_media",
                False,
                lambda: "Show banner as a quoted webpage preview instead of a"
                " regular attachment",
            ...
            loader.ConfigValue(
                "invert_media",
                False,
                lambda: "Show the banner below the text instead of above it",
                validator=loader.validators.Boolean(),
            ),
        )

    def _banner_kwargs(self) -> dict:
        """Build the file/quote/invert kwargs shared by .info and .ping banners"""
        if not self.config["banner_url"]:
            return {}

        file = self.config["banner_url"]
        if self.config["quote_media"]:
    ...
    async def infocmd(self, message: Message):
        ...
        elif self.config["banner_url"]:
            banner_kwargs = self._banner_kwargs()
